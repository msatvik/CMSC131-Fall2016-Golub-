 ...
 * to load an image from the local machine or using an Internet URL.
 * 
			...
			try {
				...
				while (true) {
					...
					if (radioButtons[0].isSelected()) {
						...
						displayedPhoto = PhotoTools.isolateColor(displayedPhoto,
								0);
					} else if (radioButtons[4].isSelected()) {
						displayedPhoto = PhotoTools.isolateColor(displayedPhoto,
								1);
					} else if (radioButtons[6].isSelected()) {
						displayedPhoto = PhotoTools
								.makeGrayscale(displayedPhoto);
					} else if (radioButtons[8].isSelected()) {
						displayedPhoto = PhotoTools
								.makeArtistic(displayedPhoto);
					} else if (radioButtons[1].isSelected()) {
						displayedPhoto = PhotoTools.stretched(displayedPhoto,
								0);
						...
						displayedPhoto = PhotoTools
								.makeDoubleWithMirror(displayedPhoto);
